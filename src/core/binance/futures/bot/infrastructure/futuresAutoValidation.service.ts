...
type FuturesAutoBotOpenClawValidationOptions = {
  bypassCache?: boolean;
};

type CachedOpenClawValidation = {
  expiresAt: number;
...
function getCacheTtlMs(error: unknown) {
  if (error instanceof Error && /quota exceeded/i.test(error.message)) {
    return QUOTA_ERROR_CACHE_TTL_MS;
  }
...
async function createOpenClawValidationLogDir() {
  ...
  const logDir = join(logsRoot, `openclaw-validation-${formatValidationLogStamp(timestamp)}-${logId}`);
...
async function writeOpenClawValidationRequestLog(logDir: string, snapshot: CoinValidationSnapshot, timestamp: Date) {
  await writeFile(
    ...
    JSON.stringify(
      {
        prompt: openClawPromptInstructions,
        snapshot,
...
async function writeOpenClawValidationErrorLog(logDir: string, error: unknown) {
  const errorPayload =
    error instanceof Error
      ...
      : {
          message: 'Unknown OpenClaw validation error.',
          value: error,
        };
...
function getVolatilityState(price: number | null, atr14: number | null) {
  if (price === null || atr14 === null || price <= 0) {
...
async function runOpenClawValidation(snapshot: CoinValidationSnapshot) {
  ...
  try {
    console.log('[openclaw validation] request', {
      snapshot,
    });
    ...
    const rawOutput = await new Promise<string>((resolve, reject) => {
      ...
      child.on('close', (code, signal) => {
        ...
        reject(new Error(`openclaw agent exited with code ${code ?? 'unknown'}${suffix}${stderr ? `: ${stderr.trim()}` : ''}`));
      });
    });
    ...
    console.error('[openclaw validation] outcome', {
      error: errorPayload,
      logDir,
    });
...
function toValidationSnapshot(input: FuturesAutoBotOpenClawValidationInput): CoinValidationSnapshot {
  ...
  const timeframeSupportResistance = input.timeframeSnapshots.map((snapshot) => ({
  ...
  const currentTrend = input.timeframeSnapshots.find((item) => item.interval === '1h')?.trend ??
    input.timeframeSnapshots[0]?.trend ?? {
      ...
      structurePattern: 'Mixed' as const,
  ...
  if (!snapshot) {
    return {
      ...
      data_quality: {
        candle_consistency: false,
        has_null_values: true,
        indicator_validity: false,
        is_complete: false,
      },
      exchange: 'binance',
...
function normalizeValidatedSetup(setup: CoinValidationSnapshotSetupCandidate): OpenClawValidatedSetup {
  ...
  return {
    ...
    risk_reward: {
      tp1: setup.risk_reward.tp1 ?? 0,
      tp2: setup.risk_reward.tp2 ?? 0,
    },
    setup_type: setup.setup_type,
    stop_loss: setup.stop_loss ?? 0,
    take_profit: {
      tp1: setup.take_profit.tp1 ?? 0,
      tp2: setup.take_profit.tp2 ?? 0,
    },
  };
}
...
function createRejectedResult(
  ...
  details?: Partial<
    Pick<
      ...
      'adjustment_notes' | 'next_action' | 'suggested_setup'
    >
  >
): FuturesAutoBotOpenClawValidationResult {
  return {
...
function parseValidationResult(snapshot: CoinValidationSnapshot, rawResponse: string): FuturesAutoBotOpenClawValidationResult {
  try {
    ...
    return {
      ...
      reason: response.rejection_reasons.length > 0 ? response.rejection_reasons.join(' ') : 'OpenClaw accepted the setup.',
      setup_id: snapshot.setup_id,
      next_action: response.next_action,
      suggested_setup: response.suggested_setup,
    ...
    const errorMessage = error instanceof Error ? error.message : 'OpenClaw returned an unreadable validation response.';
    return createRejectedResult(snapshot, errorMessage);
  }
}
  ...
  async validateSetup(
    ...
    const validationPromise = (async () => {
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown OpenClaw validation error.';
        const rejectedResult = createRejectedResult(snapshot, `OpenClaw validation unavailable: ${errorMessage}`);
