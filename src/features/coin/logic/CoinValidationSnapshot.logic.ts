...
const VALIDATION_RULES: CoinValidationRules = {
  min_rr: 2.0,
...
function getSession(value: Date): CoinValidationSession {
...
function getVolatilityState(price: number | null, atr14: number | null): CoinValidationVolatilityState {
  if (!isFiniteNumber(price) || !isFiniteNumber(atr14) || price <= 0) {
    return 'normal';
  }
...
function getRiskReward(
  ...
  if (!isFiniteNumber(entryPrice) || !isFiniteNumber(stopLoss) || !isFiniteNumber(targetPrice)) {
    return null;
  }
...
function toValidationCandle(candle: FuturesKlineCandle): CoinValidationSnapshotCandle {
  return {
    ...
    low: candle.low,
    open: candle.open,
    open_time: String(candle.openTime),
    quote_asset_volume:
      candle.quoteAssetVolume ?? (isFiniteNumber(candle.close) && isFiniteNumber(candle.volume) ? candle.close * candle.volume : null),
...
function buildTimeframeSnapshot(params: {
  ...
  const distanceToSupport = isFiniteNumber(currentPrice) && isFiniteNumber(support) ? Math.abs(currentPrice - support) : null;
  ...
  const allAboveEma = emaValues.length > 0 && emaValues.every((ema) => isFiniteNumber(currentPrice) && currentPrice > ema);
  const allBelowEma = emaValues.length > 0 && emaValues.every((ema) => isFiniteNumber(currentPrice) && currentPrice < ema);
...
function buildSetupCandidate(params: {
  ...
  return {
    ...
    distance_to_resistance:
      isFiniteNumber(currentPrice) && isFiniteNumber(currentResistance) ? Math.abs(currentResistance - currentPrice) : null,
    distance_to_support:
...
function hasNullishValue(value: unknown, visited = new Set<object>()): boolean {
  ...
  if (Array.isArray(value)) {
    return value.some((item) => hasNullishValue(item, visited));
  }
...
function isTimeframeConsistent(timeframe: CoinValidationSnapshotTimeframe, expectedLength: number) {
  if (timeframe.candles.length !== expectedLength) {
    return false;
  }

  if (!timeframe.candles.every((candle, index, array) => index === 0 || Number(candle.open_time) >= Number(array[index - 1]?.open_time))) {
    return false;
  }
